...
trace static void embed_force3(coord *Fp, coord *Fmu) {
  coord Fps = {0}, Fmus = {0};
  foreach (reduction(+ : Fps) reduction(+ : Fmus)) {
	...
	if (constant(mu.x) != 0.) {
	  ...
	  foreach_dimension() Fmus.x -=
	      area * mua *
	      (dudn.x * (sq(n.x) + 1.) + dudn.y * n.x * n.y +
	       dudn.z * n.x * n.z);
	}
      }
  }
...
int main(int argc, char **argv) {
  ...
  while (*++argv != NULL && argv[0][0] == '-')
    ...
    case 'h':
      fprintf(
	  stderr,
	  "Usage: cylinder [-h] [-v] [-F] [-i] -r <Reynolds number> "
	  ...
	  "  -F          Output the full field data\n"
	  "  -i          Initialize velocities from the dump file\n"
	  "  -b <string> Boundary conditions for 'top' and 'front' surfaces\n"
  ...
  if (dump_path == NULL && !MinLevelFlag) {
    fprintf(stderr, "cylinder: error: -l is not set\n");
    exit(1);
  }
  if (dump_path == NULL && !MaxLevelFlag) {
